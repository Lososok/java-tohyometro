...
public class ApplicationServiceImpl implements ApplicationService {
    ...
    private boolean parseStation(ArrayList<Line> lines, ArrayList<Station> stations, String parsLine) {
        ...
        Line line = lines.stream().anyMatch(x -> x.getName().equals(split[0])) ?
                ...
                new Line(split[0]);
        if (stations.stream().anyMatch(x -> x.getName().equals(split[1]) && x.getLine().equals(line))) return false;
        // repeating the code, I know
        switch (split[2]) {
            case "NonCrossing":
                var station = new NonCrossingStation(split[1], line);
            ...
            case "Crossing":
                ...
                for (int i = 3; i < split.length; i++) {
                    ...
                    if (crossLine == null ||
                            crossStation == null ||
                            crossStation.getType() != Type.Crossing) return false;
            ...
            case "Transfer":
                ...
                for (int i = 3; i < split.length; i++) {
                    ...
                    if (transferLine == null ||
                            ...
                            transferStation.getType() != Type.Transfer) return false;
        ...
        if (lines.stream().noneMatch(x -> x.getName().equals(split[0]))) lines.add(line);
        return true;
    }
}
